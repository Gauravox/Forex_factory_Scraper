...
CSV_FILE   = "forex_factory_calender.csv"
...
def get_last_scraped_date():
    ...
    with open(CSV_FILE, "r", encoding="utf-8") as f:
        ...
        # Attempt both formats
        for fmt in ("%Y-%m-%dT%H:%M:%SZ", "%Y-%m-%dT%H:%M:%S"):
            try:
                last_dt = datetime.strptime(last_date_str, fmt)
                return last_dt.date() + timedelta(days=1)
            except ValueError:
                continue
        raise ValueError(f"Unrecognized date format: {last_date_str}")
